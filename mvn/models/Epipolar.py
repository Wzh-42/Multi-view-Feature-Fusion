...
#from mvn.utils import  config as cfg
from mvn.core import cfg
from mvn.vision.multiview import camera_center, normalize, de_normalize, pix2coord, coord2pix, findFundamentalMat
from mvn.utils.BN import zeroinitBN
...
class Epipolar(nn.Module):
    def __init__(self, debug=False):
        ...
        self.debug = debug
        self.downsample = cfg.BACKBONE.DOWNSAMPLE  # 4
        #print(cfg.BACKBONE.DOWNSAMPLE)
        # h, w = cfg.DATASETS.IMAGE_SIZE   # (384, 384)
        # self.feat_h, self.feat_w = h // self.downsample, w // self.downsample
        self.feat_h, self.feat_w = cfg.KEYPOINT.HEATMAP_SIZE  # (96, 96)
        self.sample_size = cfg.EPIPOLAR.SAMPLESIZE  # 64
        self.epsilon = 0.001 # for avoiding floating point error

        y = torch.arange(0, self.feat_h, dtype=torch.float) # 0 .. 95  热图的行列每个位置对应
        x = torch.arange(0, self.feat_w, dtype=torch.float) # 0 .. 95

        if cfg.EPIPOLAR.REPROJECT_LOSS_WEIGHT > 0:  # False
        ...
        # y = y * cfg.DATASETS.IMAGE_RESIZE * cfg.DATASETS.PREDICT_RESIZE # 1 * 1
        # x = x * cfg.DATASETS.IMAGE_RESIZE * cfg.DATASETS.PREDICT_RESIZE
        # x y 的数值变了 但是维度没变  x * downsample + downsample / 2.0 - 0.5
        y = pix2coord(y, cfg.BACKBONE.DOWNSAMPLE)  # 1.5 .. 381.5
        y = y * cfg.DATASETS.IMAGE_RESIZE * cfg.DATASETS.PREDICT_RESIZE   # y * 1 * 1
        x = pix2coord(x, cfg.BACKBONE.DOWNSAMPLE)   # 95 -> 381.5
        x = x * cfg.DATASETS.IMAGE_RESIZE * cfg.DATASETS.PREDICT_RESIZE
        ...
        # 将(0,1)分成64份
        self.sample_steps = torch.range(0, 1, 1./(self.sample_size-1)).view(-1, 1, 1, 1)  # (64,1,1,1)

        if cfg.EPIPOLAR.BOTTLENECK != 1:  # False
    ...
    # feat, other_features, KRT, other_KRT, camera=camera, other_camera=other_camera
    def forward(self, feat1, feat2, P1, P2, depth=None, camera=None, other_camera=None, ref1=None, ref2=None):
        ...
        assert cfg.EPIPOLAR.ATTENTION in {'avg', 'max'}  # avg
        assert cfg.EPIPOLAR.SIMILARITY in {'cos', 'dot', 'prior'}  # 'dot'
        ...
        if cfg.EPIPOLAR.FIND_CORR == 'rgb':  # 'feature'
            ...
            if 'other1' in cfg.EPIPOLAR.OTHER_GRAD:  # ('other1', 'other2')
                other1 = feat2
            else:
                other1 = feat2.detach()
            # cfg.EPIPOLAR.PARAMETERIZED = ()
            if 'phi' in cfg.EPIPOLAR.PARAMETERIZED:
                other1 = self.phi(other1)
            if 'theta' in cfg.EPIPOLAR.PARAMETERIZED:
                feat1 = self.theta(feat1)

        if 'other2' in cfg.EPIPOLAR.OTHER_GRAD: # ('other1', 'other2')
            other2 = feat2
        else:
            other2 = feat2.detach()
       ...
       #     print(ref1.shape, feat1.shape)
        ...
        # 在with torch.no_grad计算，由x得到的新tensor（命名为w-标量）requires_grad也为False，且grad_fn也为None,即不会对w求导。
        with torch.no_grad():
            if self.debug:
            ...
            #     sample_size x N x H x W x 2
            sample_locs = sample_locs.float()
        #     sample_locs可以理解为从起点start开始，隔一段距离取点 共64个点的坐标
        out = []
        corr_pos = []
        if not usedepth:
            depth = []
        for i in range(N):
            ...
            # (1, N, C_ref, H, W)
            #  other1:(sample_size, N, C_ref, H, W)   sample_locs:(sample_size ,N ,H , W , 2)
            other1_sampled = F.grid_sample(other1[:, i], sample_locs[:, i])
            if cfg.EPIPOLAR.POOLING:  # False
                stride = 2
            ...
            if usedepth:
                ...
                elif cfg.EPIPOLAR.FIND_CORR == 'rgb':  # 'feature'
            ...
            elif cfg.EPIPOLAR.ATTENTION == 'avg':  # True
                idx = sim.argmax(0)
                with torch.no_grad():
                    # H x W x 2
    ...
    def grid2sample_locs(self, grid, P1, P2, H, W):
        ...
        # N x 4 x 3
        P1t = P1.transpose(1, 2)  # (N,4,3)
        # P1inv = torch.matmul(P1t, torch.inverse(torch.matmul(P1, P1t)))
        P1inv = torch.stack([i.pinverse() for i in P1])  # 求伪逆  # (N,4,3)
        # N x 4 x HW  grid是热图大小的所有点的二维坐标的齐次形式
        ...
        # 且指定的dim维上size必须为3。如果不指定dim，则默认为第一个尺度为3的维。
        ...
        # N x HW x 4       Nintersections < 2表示极线与图片区域没有交点或者交点在其中一个角上
        ...
        # N x HW x 2 x 2  这就是为什么要把tmp_mask[Nintersections < 2]的部分不全部变为0 而是取两个true
        # 是为了保证valid_intersections每次都能取到两个点对应的坐标
        # N x HW x 4 x 2[N x HW x 4]  # tmp_mask是非0即1的 即对于intersections的最后一个维度只取其中一个
        # (N, H*W, 4).view(N, H*W, 2, 2)
        
        # ------------------------ayy-----------------------
        print("tmp_mask:",tmp_mask.shape)
        print("tmp_mask[Nintersections < 2]:",tmp_mask[Nintersections < 2].shape)
        print("intersections[tmp_mask]:",intersections[tmp_mask].shape)
        valid_intersections = intersections[tmp_mask][:N*H*W*2,:].view(N, H*W, 2, 2)
        print("intersections[tmp_mask]:",intersections[tmp_mask].shape)
# ------------------------ayy------------------
        # valid_intersections = intersections[tmp_mask].view(N, H*W, 2, 2)
        # 没有交集的部分变为无效值  xmin-10000 ymin-10000 xmax-10000 ymax-10000
        ...
        # sample_size x N x HW x 2   sample_steps:(64,1,1,1)
        ...
        # sample_size*N x H x W x 2  将x、y的坐标变为(-1,1)
