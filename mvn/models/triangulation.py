...
# ------------------ayy------------------
from mvn.models.Epipolar import Epipolar
from mvn.models.Attention import CGCA_branch
...
class VolumetricTriangulationNet(nn.Module):
    def __init__(self, config, device='cuda:0'):
        ...
        # volume
        self.volume_softmax = config.model.volume_softmax  # true
        self.volume_multiplier = config.model.volume_multiplier  # 1.0
        self.volume_size = config.model.volume_size  # 64

        self.cuboid_side = config.model.cuboid_side  # 2500.0
        ...
        self.heatmap_multiplier = config.model.heatmap_multiplier  # 100
...
#         ---------------------ayy--------------------- 改变特征融合块插入位置
    ...
    def forward(self, images, proj_matricies, batch):
        ...
        # ----------------------ayy-------------------------------
        heatmaps = self.generate_layer(features)
        print("heatmaps:",heatmaps.shape)
        # ----------------------ayy-------------------------------

        # ----------------------ayy-------------------------------
        fc = self.CGCA(features)
        fc = fc.view(batch_size, n_views, *fc.shape[1:])
        print("fc:",fc.shape)
        # ----------------------ayy-------------------------------
        ...
        keypoints_2d_transformed[:, :, :, 1] = keypoints_2d[:, :, :, 1] * (image_shape[0] / heatmap_shape[0])
        keypoints_2d = keypoints_2d_transformed
        # print("alg_confidences:",alg_confidences)
        keypoints_3d = multiview.triangulate_batch_of_points(
            proj_matricies, keypoints_2d,)
        #  confidences_batch=alg_confidences
        ...
        features_otherview = torch.flip(features,dims=[1])
        proj_matricies_otherview = torch.flip(proj_matricies,dims=[1])

        features = features.view(batch_size, n_views, *features.shape[2:])

        # feat1 = features.view(-1,*features.shape[2:])
        # feat2 = features_otherview.view(-1, *features.shape[2:])
        # P1 = proj_matricies.reshape(-1,*proj_matricies.shape[2:])
        # P2 = proj_matricies_otherview.reshape(-1,*proj_matricies.shape[2:])
        feat1 = features.clone()
        feat2 = features_otherview.clone()
        P1 = proj_matricies
        P2 = proj_matricies_otherview
        ff = features.clone()
        for i in range(features.shape[1]):
            f,_,_,_ = self.epipolar(feat1[:,i,:,:,:],feat2[:,i,:,:,:],P1[:,i,:,:],P2[:,i,:,:], depth=None, camera=None, other_camera=None, ref1=None, ref2=None)
            ff[:,i,:,:,:] = f
...
#         ----------------------ayy------------------------
        ...
        base_points = keypoints_3d[:, 6, :3].to(device)
        for batch_i in range(batch_size):
            if self.kind == "coco":
                base_point = (keypoints_3d[11, :3] + keypoints_3d[12, :3]) / 2
            elif self.kind == "mpii":
                # ------------------ayy加了batch_i--------------------
                base_point = keypoints_3d.cpu().detach().numpy()[batch_i,6, :3]
                # print("base_point是否反传：",base_point.requires_grad)
        ...
        # -------------------------ayy加了一个keypoints_3d------------------------------------
        return vol_keypoints_3d, features, volumes, vol_confidences, cuboids, coord_volumes, base_points,keypoints_3d
