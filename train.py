...
from mvn.utils import img, multiview, op, vis, misc, cfg  #utils就是存放自己写好的自定义函数的包
...
def parse_args():  #传入参数
    ...
    parser.add_argument("--config", type=str, required=True, help="Path, where config file is stored") #配置文件
    parser.add_argument('--eval', action='store_true', help="If set, then only evaluation will be done") #评价指标
    parser.add_argument('--eval_dataset', type=str, default='val', help="Dataset split on which evaluate. Can be 'train' and 'val'") #评价数据

    parser.add_argument("--local_rank", type=int, help="Local rank of the process on the node")
    parser.add_argument("--seed", type=int, default=42, help="Random seed for reproducibility") #随机重复
...
def setup_human36m_dataloaders(config, is_train, distributed_train):
    ...
    if is_train:  #加载训练数据集Human3.6
        ...
        train_sampler = torch.utils.data.distributed.DistributedSampler(train_dataset) if distributed_train else None #将数据加载到数据集子集的采样器。

        train_dataloader = DataLoader( #数据集+取样器，把数据分组————————训练数据集
            train_dataset,
    ...
    # val
    val_dataset = human36m.Human36MMultiViewDataset(  #验证数据集
    ...
    val_dataloader = DataLoader(  #验证数据集取样器
        val_dataset,
    ...
    return train_dataloader, val_dataloader, train_sampler #sampler采样器


def setup_dataloaders(config, is_train=True, distributed_train=False): #config配置
...
def setup_experiment(config, model_name, is_train=True):  #建立实验
    prefix = "" if is_train else "eval_"  #prefix前缀
...
def one_epoch(model, criterion, opt, config, dataloader, device, epoch, n_iters_total=0, is_train=True, caption='', master=False, experiment_dir=None, writer=None):
        ...
        for iter_i, batch in iterator:
            with autograd.detect_anomaly():
                ...
                if model_type == "alg" or model_type == "ransac":
                    ...
                    # ---------------ayy加了keypoints_3d-------------------
                    keypoints_3d_pred, heatmaps_pred, volumes_pred, confidences_pred, cuboids_pred, coord_volumes_pred, base_points_pred,keypoints_3d = model(images_batch, proj_matricies_batch, batch)
                ...
                #-------------------ayy------------------
                total_loss = 0.0
                loss1 = criterion(keypoints_3d_pred * scale_keypoints_3d, keypoints_3d_gt * scale_keypoints_3d,
                                  keypoints_3d_binary_validity_gt)
                loss2 = criterion(base_points_pred * scale_keypoints_3d, keypoints_3d_gt[:, 6, :] * scale_keypoints_3d,
                                  keypoints_3d_binary_validity_gt[:, 6, :])
    ...
    # calculate evaluation metrics计算评估指标
    if master:
        if not is_train:
        ...
        # dump to tensorboard per-epoch stats 转储到tensorboard每个历元的统计数据
        for title, value in metric_dict.items():
...
##该模块用以配置GPU环境等
def init_distributed(args):  ##初始化各进程环境
    if "WORLD_SIZE" not in os.environ or int(os.environ["WORLD_SIZE"]) < 1: #获取环境变量
        return False

    torch.cuda.set_device(args.local_rank)  #指定显卡

    assert os.environ["MASTER_PORT"], "set the MASTER_PORT variable or use pytorch launcher" #assert判断表达式是否为真；否则输出参数并提示错误信息“设置MASTER_PORT变量或使用pytorch启动器
    assert os.environ["RANK"], "use pytorch launcher and explicityly state the rank of the process" #使用pytorch发射器并明确说明过程的顺序

    torch.manual_seed(args.seed)  #为CPU中设置种子，生成随机数
    torch.distributed.init_process_group(backend="nccl", init_method="env://") ##分布式训练

    return True


def main(args):  #主程序
    print("Number of available GPUs: {}".format(torch.cuda.device_count()))  #可用的GPU个数

    is_distributed = init_distributed(args) #已分配is_distributed （init_distributed 初始分配的）
    master = True
    if is_distributed and os.environ["RANK"]:
        master = int(os.environ["RANK"]) == 0

    if is_distributed:
        device = torch.device(args.local_rank)  #分配到设备型号
    else:
        device = torch.device(0)

    # config
    config = cfg.load_config(args.config)  #配置
    config.opt.n_iters_per_epoch = config.opt.n_objects_per_epoch // config.opt.batch_size

    model = {
        "ransac": RANSACTriangulationNet, #从文件夹MVN中输入的模型"三角网"
        "alg": AlgebraicTriangulationNet,
        "vol": VolumetricTriangulationNet
    }[config.model.name](config, device=device).to(device)  # 将模型加载到指定设备上

    if config.model.init_weights: #加载权重
        ...
        print("Successfully loaded pretrained weights for whole model") #加载权重

    # criterion  #评价指标
    criterion_class = {
        "MSE": KeypointsMSELoss,  #从MVN中输入loss函数
...
#                  --------------ayy------------------
                 {'params': model.CGCA.parameters()},
                 {'params': model.generate_layer.parameters()},
#                  --------------ayy------------------
    ...
    if not args.eval:
        ...
        for epoch in range(config.opt.n_epochs):
            if train_sampler is not None:  #sampler采样器
